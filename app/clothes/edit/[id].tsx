...
function BrandModal({ visible, brands, onSelect, onClose }: {
    visible: boolean;
    brands: string[];
    onSelect: (val: string) => void;
    onClose: () => void;
...
export default function EditClothesScreen() {
    ...
    useEffect(() => {
        ...
        const item = getClothesById(numId) as any;
        if (item) {
            setName(item.name ?? '');
            setCategory(item.category ?? '');
            setMaterial(item.material ?? '');
            setWashInstruction(item.wash_instruction ?? '');
            setColor(item.color ?? '');
            setBrand(item.brand ?? '');
        }
    ...
    function handleSave() {
        ...
        const numId = Number(Array.isArray(id) ? id[0] : id);
        updateClothes(numId, name, category, material, washInstruction, color, brand);
        router.back();
    }

    return (
        <ScrollView style={styles.container}>
            <Text style={styles.title}>Bearbeiten</Text>
            ...
            <BrandModal
                ...
                onSelect={(val) => {
                    setBrand(val);
                    setBrands(getAllBrands().map((b: any) => b.name));
                }}
...
const styles = StyleSheet.create({
    ...
    selectorPlaceholder: { fontSize: 15, color: 'white' },
    button: {
